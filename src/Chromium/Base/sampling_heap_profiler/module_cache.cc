...
const ModuleCache::Module* ModuleCache::GetModuleForAddress(uintptr_t address) {
  ...
  std::shared_ptr<Module> new_module = CreateModuleForAddress(address);
  if (!new_module)
    return nullptr;
...
void ModuleCache::AddNonNativeModule(std::shared_ptr<Module> module) {
  DCHECK(!module->IsNative());
  non_native_modules_.push_back(std::move(module));
}

void ModuleCache::InjectModuleForTesting(std::shared_ptr<Module> module) {
...
ModuleCache::Module* ModuleCache::FindModuleForAddress(
  ...
  auto it = std::find_if(modules.begin(), modules.end(),
                         [address](const std::shared_ptr<Module>& module) {
