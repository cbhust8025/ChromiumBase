...
#include "lock_free_address_hash_set.h"
...
LockFreeAddressHashSet::~LockFreeAddressHashSet() {
  for (std::shared_ptr<std::atomic<Node*>>& bucket : buckets_) {
...
void LockFreeAddressHashSet::Insert(void* key) {
  ...
  std::shared_ptr<std::atomic<Node*>>& bucket = buckets_[Hash(key) & bucket_mask_];
  Node* node = bucket->load(std::memory_order_relaxed);
...
void LockFreeAddressHashSet::Copy(const LockFreeAddressHashSet& other) {
  DCHECK_EQ(0u, size());
  for (const std::shared_ptr<std::atomic<Node*>>& bucket : other.buckets_) {
